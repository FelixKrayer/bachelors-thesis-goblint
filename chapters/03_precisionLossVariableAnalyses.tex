...
\chapter{Combatting the Precision Loss of Variable Analyses}\label{chapter:precisionLossVariableAnalyses}
  In this chapter we describe our approach to reduce the precision loss described in \autoref{section:precisionLoss} for analyses tracking information in relation to variables. We will call analyses of this kind "variable analyses".\\
  ...
  In the following we call a variable that has been written or altered in the current procedure context "tainted". Therefore, we introduce a new taint analysis tracking which variables have been tainted within the context of the current procedure in the following section. It is worth mentioning that our notion of taintedness is related but different from other uses of the term "taint analysis".\\
    ...
    \subsection{Improving variable analyses}\label{section:formalImprove}
    ...
    Recall the source of the precision loss we want to reduce as described in \autoref{section:precisionLoss}. This happened when a global variable was updated with a less precise value after a procedure call even though this specific variable was not changed by the call.\\
  ...
  \end{figure}
    ...
    In this section we discuss how we improved other existing analyses in \gob\ using the taint analysis we implemented in \autoref{section:implTaint}.
    \subsubsection{Improving the \texttt{base} analysis}
      ...
      %TODO: Sumarize the updated combine
